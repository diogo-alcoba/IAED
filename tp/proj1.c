...
/* Descricao: Um sistema de logistica que permite gerir stocks de produtos e encomendas */
...
void lista_cliente(char cmd[]);
void ordena_custo();
void mergesort_custo(Encomenda vetor[], int esq, int dir);
void merge_custo(Encomenda vetor[], int esq, int m, int dir);
int compara_menor_custo(Encomenda p1, Encomenda p2);
...
Encomenda aux_custo[idmaxen];

/* vai ler o input do utilizador e executar o comando devido  sdf*/
int main(){
    char cmd[cmdmax]={0};
    while (cmd[0]!='x'){
        scanf(" %[^\n]", cmd);
...
void cria_encomenda(char cmd[]){
    Encomenda res = {0};
    sscanf(cmd, "N %[^\n]", res.cliente);
    res.id = indexiden;
    idencomenda[indexiden] = res;
    printf("Nova encomenda %d %s.\n", indexiden, res.cliente);
    indexiden++;
}
...
void ordena_custo(){
    Encomenda res[idmaxp_en]={0};
    int i;
    for (i=0; i<indexiden; i++)
        res[i]=idencomenda[i];
    mergesort_custo(res, 0, indexiden-1);
    puts("Encomendas");
    for (i=0; i<indexiden; i++)
        printf("* %d %d\n", res[i].id, res[i].preco);
}
...
void mergesort_custo(Encomenda vetor[], int esq, int dir){
    ...
    if (dir<=esq) 
        return;
    mergesort_custo(vetor, esq, m);
    mergesort_custo(vetor, m+1, dir);
    merge_custo(vetor, esq, m, dir);
}

void merge_custo(Encomenda vetor[], int esq, int m, int dir){
    int i, j, k;
    for (i=m+1; i>esq; i--)
        aux_custo[i-1]=vetor[i-1];
    for (j=m; j<dir; j++)
        aux_custo[dir+m-j]=vetor[j+1];
    for (k=esq; k<=dir; k++){
        if (compara_menor_custo(aux_custo[j], aux_custo[i]) || i>m)
            vetor[k]=aux_custo[j--];
        else
            vetor[k]=aux_custo[i++];
    }
