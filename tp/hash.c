...
/* Descricao: Um sistema de gestão de uma base de dados de jogos de futebol amigáveis. */
...
pLst_Ord inicializa_lista_ordenada(pLst_Ord lista_ordenada){
    ...
    lista_ordenada->lista_equipas = (pEquipa*) calloc(MAX_EQUIPAS, sizeof(pEquipa));
...
void elimina_equipa_hash(pListas lista_equipas, char* nome){
    link_equipa prev, link;
    for (link = lista_equipas->head_equipas, prev = NULL; link != NULL; prev = link, link = link->next){
        if (!strcmp(link->equipa->nome, nome)){
            if (link == lista_equipas->head_equipas)
                lista_equipas->head_equipas = link->next;
            else
                prev->next = link->next;
            if (link == lista_equipas->tail_equipas)
                lista_equipas->tail_equipas = prev;
...
void adiciona_jogo(int NL, pListas* listas, pLst_Ord lista_ordenada, int m){
    ...
    scanf(" %[^:]:%[^:]:%[^:]:%d:%d:%[^\n]", nome, equipa_1, equipa_2, &pontuacao_1, &pontuacao_2, informacao);
    index = hashU(nome, m);
    ...
    novo_jogo_hash->jogo->informacao = str_dup(informacao);
...
void consulta_jogo(int NL, pListas* listas, int m){
    ...
    printf("%d %s %s %s %d %d %s\n", NL, nome, link->jogo->equipa_1, link->jogo->equipa_2, pontuacao_1, pontuacao_2, link->jogo->informacao);
...
void jogos_ordenados(int NL, pLst_Ord lista_ordenada){
    ...
    for (link = lista_ordenada->head_jogos; link != NULL; link = link->next){
        printf("%d %s %s %s %d %d %s\n", NL, link->jogo->nome, link->jogo->equipa_1, link->jogo->equipa_2, link->jogo->pontuacao[0], link->jogo->pontuacao[1], link->jogo->informacao);
    }
    return;
}
...
void remove_equipa(int NL, pListas* listas, pLst_Ord lista_ordenada, int m){
    char nome[1023];
    int index;
    link_equipa link_equipa;
    link_jogo link_jogo;
    scanf(" %[^\n]", nome);
    link_equipa = procura_equipa(listas, nome, m);
    if (link_equipa == NULL){
        printf("%d Equipa inexistente.\n", NL);
        return;
    }
    for (link_jogo = lista_ordenada->head_jogos; link_jogo != NULL; link_jogo = link_jogo->next){
        if (!strcmp(link_jogo->jogo->equipa_1, nome) || !strcmp(link_jogo->jogo->equipa_2, nome)){
            printf("%d Equipa com jogos.\n", NL);
            return;
        }
    }
    index = hashU(nome, m);
    elimina_equipa_hash(listas[index], nome);
    return;
}
