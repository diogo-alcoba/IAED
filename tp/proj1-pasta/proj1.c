...
/* Limites */
#define stringmax 64  /* maximo de caracteres na descricao dum produto */
...
/* int compara_custo(Encomenda p1, Encomenda p2);
void ordena_custo(char cmd[]); */
...
/* void ordena_custo(char cmd[]){
    ...
    for (i=0; i<indexiden; i++)
        res[i]=idencomenda[indexiden];
    mergesort(res, 0, idencomenda[indexiden], CMPcusto);
     for (i=0; i<indexiden; i++)
...
void merge(Produto vetor[], int esq, int m, int dir, int cmp){
    ...
    if (cmp == CMPdescricoes || cmp == CMPprecos){
        for (k=esq; k<=dir; k++){
   ...
   /*  elif (cmp == CMPcusto){
        for (k=esq; k<=dir; k++){
            if (compara_custo(aux[j], aux[i]) || i>m)
                vetor[k]=aux[j--];
            else
                vetor[k]=aux[i++];
    } */
